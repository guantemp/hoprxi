<template>
	<view class="price_adjustment_sheet">
		<hop-nav-bar title="订单" :backgroundColor="[1, ['#FD6E6A', '#FFC600', 90]]" :titleFont="['#FFF','left',700]"
			surplusHeight=43 id="navBar">
			<view slot="extendSlot" class="cu-bar search">
				<view class="search-form radius">
					<text class="cuIcon-search"></text>
					<input v-model="scanResult" :adjust-position="false" type="text" placeholder="请输入单据号"
						confirm-type="search">
				...
				<view class="sheet">
					...
					<view class="sheetDetailed margin-top-xs padding-lr-xs" @tap.stop="navToSheet(sheet.sheetNumber)">
						...
						<view class="flex flex-direction midlle text-cut">
							...
							<text class="margin-bottom-xs">确认日期：{{sheet.effectDate}}</text>
		...
		<!-- bootom -->
		<view class="add">
			<button class="cu-btn lg radius shadow bg-yellow basis-xl"
				@tap.stop="this.$util.navTo('/pages/workflow/price/price_adjustment_add?sign=add')">
				<text class="cuIcon-add margin-right-xl"></text>新增订单</button>
		</view>
		<!-- 删除对话框 -->
