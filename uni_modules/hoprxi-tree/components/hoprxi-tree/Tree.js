import HashMap from '@/common/js/HashMap.js';
//节点
export class Node {
...
export class Tree {
	constructor(root, isCheckOnlyLeaf, nodeCheckType) {
		if (!root || !root instanceof Node) throw new Error('parent and child required Node class');
		this.root = root;
		this.root.visible = true;
		this.nodeHashMap = new HashMap();
		this.isCheckOnlyLeaf = false;
		this.nodeCheckType = 'indeterminate';
		this.nodeHashMap.put(root.id, this.root);
	}
	/*
	 * @description 增加子元素
	 * @param {parent} 父亲节点
	...
	addChild(parent, child) {
		if (!parent instanceof Node || !child instanceof Node) throw new Error(
			'parent and child required Node class');
		const _sibling = (previous, next) => {
			let previousNode = this.nodeHashMap.get(previous);
			if (previousNode) {
		...
		parent.isLeaf = false;
		this.nodeHashMap.put(child.id, child);
	...
	parent(node) {
		if (node) return this.nodeHashMap.get(node.parentId);
		return null;
	}
	siblings(sibling) {
		if (!sibling instanceof Node) throw new Error('sibling required Node class');
		if (sibling.parentId) {
			let parent = this.nodeHashMap.get(sibling.parentId);
			return children(parent);
		}
		return [];
	}
	children(parent) {
		if (!parent instanceof Node) throw new Error('parent required Node class');
		const _child = (children, next) => {
			let nextNode = this.nodeHashMap.get(next);
	...
	getNode(id) {
		return this.nodeHashMap.get(id);
	}
	path(node) {
		let path = [];
		const _path = (path, node) => {
			if (node.parentId) {
				let temp = this.nodeHashMap.get(node.parentId);
				path.push(temp);
	...
	depth(key) {
		...
		let a = 0;
		let b = 0;
		let node = this.nodeHashMap.get(key);
		if (node) {
	...
	presetExpanded(expandIds = []) {
		const _sibling = (node) => {
			if (node) {
				node.visible = true;
				_sibling(this.nodeHashMap.get(node.next));
			}
		}
		const _primordial = (node) => {
			let parent = this.nodeHashMap.get(node.parentId);
			if (parent) {
				...
				_sibling(this.nodeHashMap.get(parent.firstChild)); //兄弟节点显示，不处理兄弟的字节点
				_primordial(parent);
			}
		}
		for (const id of expandIds) {
			let node = this.nodeHashMap.get(id);
			//本节点已显示的不在处理,并不处理子节点
	...
	presetChecked(checkedIds = []) {
		...
		for (const id of checkedIds) {
			let basic = this.nodeHashMap.get(id);
			if (basic) {
